...
1. **不明点は都度ユーザーに質問する** — 推測で進めない
2. **提案ベースで進める** — 「こうしたほうがいいんじゃないか」を競合・参考サイトを元に積極的に提案
3. **参考サイトがあれば共有する** — 他サービスのUI/UXを参考にした場合、URLやスクショで根拠を示す
4. **Plan Mode で全体設計を先に見せる** — 大きめの実装は着手前にユーザー承認を取る

## パフォーマンス方針

- **SSR優先**: コンポーネントはサーバーコンポーネントをデフォルトとし、`'use client'` は必要最小限に
- **analytics トラッキング**: `data-track-*` 属性 + `AnalyticsTracker`（グローバルイベントデリゲーション）を使い、トラッキングのためだけにコンポーネントをクライアント化しない
- **画像最適化**: Next.js Image の `loading="lazy"` を活用し、ビューポート外の画像は遅延ロード
