...
### 🔴 9/6夕方の追記: 池袋は未インデックスだった（前提が変わった）

GSC再クロール依頼の際に判明。`index_inspect` 実測:

| ページ | インデックス | 最終クロール |
|---|---|---|
| **ikebukuro** | ❌ **クロール済み - インデックス未登録** | **3/16**（約6ヶ月前） |
| shinjuku-kagurazaka | ✅ 登録済み | 6/1 |
| shibuya-ebisu-daikanyama | ✅ 登録済み | 6/1 |
| roppongi-azabu | ✅ 登録済み | 7/7 |

**この指示書の「池袋241PVの源泉は順位3.2位そのもの」という記述は誤り。**
インデックスされていないページが3位に出ることはない。241PVは内部回遊のみで、
「池袋 個室サウナ」3.2位を受けているのは別ページ（/area/tokyo か施設ページ）。
また3/16以降クロールされていないため、8/29のarea修正も9/6の変更もGoogleは見ていない。

対照群の設計も想定と逆だった。池袋は「既に3位の強いページ」ではなく**最も条件の悪いページ**で、
素直な検証対象は新宿・渋谷（6/1クロール・インデックス済み）。

判定（優先順に）:
1. **最重要: 池袋がインデックスされたか**。されなければ「コンテンツを足してもインデックスに値しないと判断されている」＝**施策3の方向性そのものを見直す**
2. 各サブエリアページが GSC ページ別に **imp 50以上で出現**したら「検索の受け皿化」成功 → 残り3エリアへ展開
3. `/area/tokyo` の PV（749）が減っていたら共食い。サブエリア合計と足して増えていれば OK
4. インデックス済みの新宿・渋谷・六本木で imp が動いたかを、池袋と切り分けて見る
